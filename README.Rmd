...
si2$urop
```
This exactly matches what is reported in equation (37) on page 57 in Ochoa (1984). Now let us compute the vector of labor values:
```{r}
si2$lvalues
```
...
si2$dp
```
These are different from what is reported in equation (11) on page 48 in Ochoa (1984). The reason for this difference is that he uses a vector of market prices, `(4, 60, 7)` for normalization that is different from a vector of 1s (which is the common assumption in real world analysis). 

What is the vector of prices of production?
```{r}
si2$pp
```
These are different, once again because of the different normalization, from what is reported in equation (38) on page 57 in Ochoa (1984). 
